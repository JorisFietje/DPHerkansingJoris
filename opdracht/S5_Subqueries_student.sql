...
SELECT naam
FROM medewerkers m
WHERE EXISTS (SELECT 1 FROM medewerkers o WHERE o.chef = m.mnr);
...
SELECT naam
FROM medewerkers m
WHERE NOT EXISTS (SELECT 1 FROM medewerkers o WHERE o.chef = m.mnr);
...
WHERE m.functie = 'TRAINER'
  AND EXISTS (SELECT 1
              FROM uitvoeringen u
                       ...
                       JOIN inschrijvingen i ON i.cursus = u.cursus
                                            AND i.begindatum = u.begindatum
              WHERE u.docent = m.mnr
                AND c.type = 'ALG'
                AND i.cursist = m.chef);
...
SELECT naam
FROM medewerkers m
WHERE NOT EXISTS (SELECT 1 FROM uitvoeringen u WHERE u.docent = m.mnr);
